...
export const getTeachers = async (req: Request, res: Response) => {
  const query = ` SELECT talla, genero, SUM(total) as totalUniformes, MAX(ultima_entrada) as ultimaEntrada FROM maestros GROUP BY talla `
  try {
...
export const createTeachers = async (req: Request, res: Response) => {
    ...
    const query = "INSERT INTO maestros (user, proveedor, coste, coste_total, total, talla, genero, fecha_registro, ultima_entrada, anotacion) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?,?)";
  ...
  export const getLastTeachers = async (req: Request, res: Response) => {
    const query = "SELECT SUM(total) as totalUniformes, MAX(ultima_entrada) as ultimaEntrada FROM maestros"
    try {
