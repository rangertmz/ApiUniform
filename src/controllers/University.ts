...
export const getUniversity = async (req: Request, res: Response) => {
  const query = ` SELECT talla, genero, SUM(total) as totalUniformes, MAX(ultima_entrada) as ultimaEntrada FROM universidad GROUP BY talla `
  try {
...
export const createUniversity = async (req: Request, res: Response) => {
    ...
    const query = "INSERT INTO universidad (user, proveedor, coste, coste_total, total, talla, genero, fecha_registro, ultima_entrada, anotacion) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
  ...
  export const getLastUniversity = async (req: Request, res: Response) => {
    const query = "SELECT SUM(total) as totalUniformes, MAX(ultima_entrada) as ultimaEntrada FROM universidad"
    try {
