...
export const getSports = async (req: Request, res: Response) => {
  const query = ` SELECT talla, genero, SUM(total) as totalUniformes, MAX(ultima_entrada) as ultimaEntrada FROM deportes GROUP BY talla `
  try {
...
export const createSports = async (req: Request, res: Response) => {
    ...
    const query = "INSERT INTO deportes (user, proveedor, coste, coste_total, total, talla, genero, fecha_registro, ultima_entrada, anotacion) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?,?)";
  ...
  export const getLastSports = async (req: Request, res: Response) => {
    const query = "SELECT SUM(total) as totalUniformes, MAX(ultima_entrada) as ultimaEntrada FROM deportes"
    try {
